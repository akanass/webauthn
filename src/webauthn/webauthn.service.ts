...
import { VerifyAssertionResponseOpts } from '@simplewebauthn/server/dist/assertion/verifyAssertionResponse';
...
export class WebAuthnService {
  ...
  startAttestation = (
    ...
    this._securityService.getLoggedInUser(session).pipe(
      ...
      map(
        (merged: [UserEntity, CredentialEntity[], string, WebAuthnConfig]) => ({
          user: merged[0],
      ...
      map(
        (_: {
          ...
          authenticatorSelection: Object.assign(
            {},
            _.config.authenticatorSelection,
            { authenticatorAttachment },
          ),
          excludeCredentials: !!_.credentials
      ...
      map((_: GenerateAttestationOptionsOpts) => generateAttestationOptions(_)),
      map(
        (_: PublicKeyCredentialCreationOptionsJSON) =>
  ...
  finishAttestation = (
    ...
    forkJoin([
      ...
      map(
        (merged: [UserEntity, WebAuthnAttestationSession, WebAuthnConfig]) => ({
          user: merged[0],
          data: merged[1],
          config: merged[2],
        }),
      ...
      map(
        (_: {
          user: UserEntity;
          data: WebAuthnAttestationSession;
          config: WebAuthnConfig;
        }) => ({
          verifyAttestationOptions: {
            ...
            expectedOrigin:
              !!_.config.useRpPort && !!_.config.rpPort
                ? `https://${_.config.rpID}:${_.config.rpPort}`
                : `https://${_.config.rpID}`,
            expectedRPID: _.config.rpID,
            requireUserVerification:
              _.config.authenticatorSelection.userVerification === 'required',
          } as VerifyAttestationResponseOpts,
          data: {
            user_id: _.user.id,
            name:
              _.data.authenticator_attachment === 'cross-platform'
                ? _.config.defaultSecurityKeyName
                : _.config.defaultBiometricName,
            user_handle: Buffer.from(_.data.user_handle),
            metadata:
              _.data.authenticator_attachment === 'platform'
                ? Object.assign(
                    {},
                    {
                ...
                : { authenticator_attachment: _.data.authenticator_attachment },
            transports:
              !!attestation.transports && attestation.transports.length > 0
                ? attestation.transports
                : _.config.defaultTransports,
          },
        }),
      ),
      mergeMap(
        (_: {
          verifyAttestationOptions: VerifyAttestationResponseOpts;
          data: Partial<Credential>;
        }) =>
          forkJoin([
            from(verifyAttestationResponse(_.verifyAttestationOptions)).pipe(
              catchError((err) =>
          ...
          !!_.verifiedAttestation.verified
            ? of(
                Object.assign({}, _.data, {
                  credential_id:
                    _.verifiedAttestation.attestationInfo.credentialID,
                  type: _.verifiedAttestation.attestationInfo.credentialType,
                  aaguid: _.verifiedAttestation.attestationInfo.aaguid,
                  user_verified:
                    _.verifiedAttestation.attestationInfo.userVerified,
                  public_key:
                    _.verifiedAttestation.attestationInfo.credentialPublicKey,
                  signature_count:
                    _.verifiedAttestation.attestationInfo.counter,
                  attestation_format: _.verifiedAttestation.attestationInfo.fmt,
                  attestation:
                    _.verifiedAttestation.attestationInfo.attestationObject,
            ...
            : throwError(
                () =>
                  new PreconditionFailedException(
                    "WebAuthn attestation can't be verified",
                  ),
              ),
      ),
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  ...
  startAssertion = (): Observable<PublicKeyCredentialRequestOptionsEntity> =>
    of(Config.get<WebAuthnConfig>('security.webauthn')).pipe(
      ...
      map((_: GenerateAssertionOptionsOpts) => generateAssertionOptions(_)),
      map(
        (_: PublicKeyCredentialRequestOptionsJSON) =>
  ...
  finishAssertion = (
    ...
    this._credentialService
      ...
      .pipe(
        ...
        mergeMap((_: VerifyAssertionResponseOpts) =>
          of(verifyAssertionResponse(_)).pipe(
            catchError((err) =>
              throwError(() => new BadRequestException(err.message)),
            ),
          ),
        ...
        mergeMap((_: VerifiedAssertion) =>
          !!_.verified
            ...
            : throwError(
                () =>
                  new PreconditionFailedException(
                    "WebAuthn assertion can't be verified",
                  ),
              ),
        ),
        mergeMap((_: VerifiedAssertion) =>
